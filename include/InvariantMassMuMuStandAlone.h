...
namespace Belle2 {
  namespace InvariantMassMuMuCalib {
    ...
    */
    std::tuple<std::vector<Eigen::VectorXd>, std::vector<Eigen::MatrixXd>, Eigen::MatrixXd, std::vector<double>>  runMuMuInvariantMassAnalysis(
      std::vector<Event> evts,
      const std::vector<double>& splitPoints);

    std::pair<Pars, Eigen::MatrixXd> getInvMassPars(const std::vector<Event>& evts, Pars pars, int bootStrap = 0);

    std::pair<Pars, Eigen::MatrixXd> getInvMassPars(const std::vector<double>& data, Pars pars);

    double getLikelihood(const std::vector<double> &data, Pars pars);
